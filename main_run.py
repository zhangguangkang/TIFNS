...
class EmbeddingLayer(nn.Module):
    ...
    def forward(self, g, h, r, norm):
        return self.embedding.weight, self.embedding(h.squeeze())
...
class DistMult(nn.Module):
    def __init__(self, train_entity_num, ookb_entity_num, train_triplets,aux_triplets,
                 in_dim, h_dim, num_rels, num_bases=-1,num_hidden_layers=1, dropout=0, use_cuda=True, reg_param=0, isSigmoid=False):
        super(DistMult, self).__init__()
        self.rgcn = RGCN(args, train_entity_num, ookb_entity_num, train_triplets,aux_triplets,
                         in_dim, h_dim, h_dim, num_rels * 2, num_bases, num_hidden_layers, dropout, use_cuda)
...
def main(args):
    ...
    train_entity_num, ookb_entity_num = load_entity_num(train_data, aux_data)
    ...
    if args.model is None:
        model = DistMult(train_entity_num,
                        ookb_entity_num,
                        train_data,
                        aux_data,
                        num_nodes,
    ...
    if args.n_epochs_aux != 1:
        ...
        total_graph = total_graph.to('cuda')
        total_node_id = total_node_id.to('cuda')
        total_rel = total_rel.to('cuda')
        total_norm = total_norm.to('cuda')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lr', type=float, default=0.002, help='learning rate (default: 0.003)')       #   0.003->0.02
    ...
    args.entity2class_path = '.\data\\fb15k\subject-10\Spectral' + '\\2340spectrale2c140.pickle'
    args.class2entity_path = '.\data\\fb15k\subject-10\Spectral' + '\\2340spectralc2e140.pickle'
